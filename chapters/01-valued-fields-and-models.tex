...
We assume that $v_K$ is discrete, $K$ is complete with respect to $v_K$ and $k$ is perfect.
Let $X$ be a smooth, projective and absolutely irreducible curve over $K$.
...
\begin{remark} \label{rem:flatIffIntegral}
    ...
    Thus $\OO_{\X,x}$ is a torsion-free module over the DVR $\OK$, hence flat by~\cite[Corollary 1.2.14]{Liu}.
...
\begin{example}\label{ex:model}
    ...
    \textcolor{red}{TBD: Normality}
\end{example}


\subsection{Valuations of Type \textrm{II}} \label{subsec:valuations-of-type-ii}
...
\begin{example}
    ...
    \begin{align}
        v_g: K[x] &\to \hat{\QQ}, \; v_g\left( \sum_i a_i x^{i} \right) = \min_i(v_K(a_i)).
    \end{align}
    Since $k(v_g) = k(\bar{x})$ (where $\bar{x}$ is the image of $x$ in $k(v_g)$), the Gauß valuation
    extends naturally and uniquely to $K(x)$, meaning that we have $v_g \in V_\textrm{II}(K(x))$.
    We will later see that $\lbrace v_g \rbrace$ defines the model $\X = \PP^1(\ZZ_p)$ of $\PP^1(\QQ_p)$.
\end{example}
...
Since $\xi$ has codimension $1$ in $\X$, the stalk $\OO_{\X,\xi}$ is a DVR\@.
...
\begin{proposition}\label{prop:WellDefOfInducedValuation}
    The valuation defined in~\ref{eq:ValuationOfComponent} is of type \textrm{II}.
\end{proposition}
...
\begin{proof}
    As the model $\X$ is normal, the stalk $\OO_{\X, \xi}$ is a DVR\@.
...
\begin{proof}
    ...
    This is an extension of DVRs and thus $\eta' \in Z'$ must be generic.
...
\begin{proof}[Sketch of the proof] \textcolor{red}{TBD.}
...
\begin{example}
    ...
    \begin{align*}
        v_Z|_{F_X} = v_g \qquad \text{\textcolor{red}{Why do we need to restrict?}}
    \end{align*}
    with respect to the $x$-coordinate.
    Using Lemma (\textcolor{red}{In the third talk}), we see that $v_Z(p) = 1$
    and $v_Z(x) = 0$, since the horizontal divisor on $x = 0$ intersects $\Bar{X}_p$ at a unique point
\end{example}


\section{The Proof of Theorem \ref{thm:bijectionModelsAndComponents}}\label{sec:the-proof-of-theorem-thm:bijectionmodelsandcomponents}
...
\subsection{Proof of the First Part}\label{subsec:proof-of-the-first-part}
...
\subsection{Proof of the Second Part}\label{subsec:proof-of-the-second-part}
...
\begin{proof}[of Proposition \ref{prop:constructing_models}~\ref{prop:constructing_models_ii}]
    ...
    such that $\X_s \cap \Supp D_i$ is a point of $Z_i$ that belongs to none of the components of $\E$.
    ...
    Indeed, \(\LL|_E \simeq \OO_E\) if $E \in \E$ since $E \cap \Supp D = \varnothing$;
    if $E \notin \E$, then $E$ is some $Z_i$ and $\deg \LL|_E \geq \deg \OO_\X(D_i)|_{Z_i} > 0$, and hence \(\LL|_E \not\simeq \OO_E\).
\end{proof}
